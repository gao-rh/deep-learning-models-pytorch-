    ...
    with torch.no_grad():
        for X1, X2, y in data_iter:
            if isinstance(X1, list):
                ...
                X2 = X2.to(device)
            y = y.to(device)
            metric.add(accuracy(net(X1, X2)[0], y), size(y))
    return metric[0] / metric[1]
...
def train_Fusion_2(net, train_iter, test_iter, num_epochs, lr, device, model_name, isini=1, isrecord=False):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (X1, X2, y) in enumerate(train_iter):
            ...
            train_l = metric[0] / metric[2]
            train_acc = metric[1] / metric[2]
            if isrecord & epoch == num_epochs - 1:
                with torch.no_grad():
                    weights = torch.cat([weight, y.reshape(-1,1)],1)
                    for lenss in range(weights.shape[0]):
                        list_weight.append(weights[lenss,:])
        #if (i + 1) % (num_batches // 5) == 0 or i == num_batches - 1:
        #draw
        animator.add(epoch + (i + 1) / num_batches,
             (train_l, train_acc, None))
    ...
    filename = './train_info/' + model_name +'.xlsx'
    if isrecord:
        dfw = pd.DataFrame(list_weight)
        dfw.to_excel('./train_info/weight.xlsx')
    try:
        df0 = pd.read_excel(filename)
        df = [df0, df]
        df = pd.concat(df)
